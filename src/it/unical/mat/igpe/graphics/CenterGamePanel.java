							...
							while(gameManager.timeUp()==false && finishMatch==false)
							{
								if(GameManager.isPause()==false)
								{
									if(roundControllerStart)
			        				{
		        						...
		        						if (!exitGame)
		        						{
		        							switch(counterOfRounds)
		        							{
		        							case 0: AudioProvider.round1();
		        							break;
		        							case 1: AudioProvider.round2();
		        							break;
		        							case 2: AudioProvider.finalRound();
		        							break;
		        							}
		        						}
...
//		        						pointsImage = imageProvider.getPoints(gameManager.getWorld().getCurrentPoints());
		        					...
		        					|| (GameManager.getWorld().getRoundMyPlayer()+GameManager.getWorld().getRoundComPlayer())==3) && (GameManager.isPause()==false))
		        			{
		        				...
		        				if(musicEnd==false)
		        				{
		        					switch(playGroundCGP)
							    	{
							    	 case 0: AudioProvider.stopArena1();
							    	 		 break;
							    	 case 1: AudioProvider.stopArena2();
					    	 		 		 break;
							    	 case 2: AudioProvider.stopArena3();
					    	 		 		 break;
							    	}
		        					
		        					AudioProvider.musicEndMatch();
		        					musicEnd=true;
		        				}
		        			}
...
// shoot 
    private static double xShoot=5;
    private static double yShoot=0;
// screen info
...
// utility object
		...
		this.addMouseListener(new MouseAdapter() {
		    ...
		    public void mouseReleased(final MouseEvent e){
			    ...
			    if (CenterGamePanel.repainterThread.isGameFinished()==true)
			    {
			    	...
			    	if(x>((int)(width*0.29)) && x<((int)(width*0.43)) && y>((int)(height*0.1)) && y<((int)(height*0.17)))
				    {
			    		repainterThread.musicEnd=false;
			    		AudioProvider.stopMusicEndMatch();
			    		switch(playGroundCGP)
				    	{
				    	 case 0: AudioProvider.stopArena1();
				    	 		 break;
	        ...
	        public void keyReleased (final KeyEvent e)
			 {
					...
					if(e.getKeyCode() == KeyEvent.VK_ENTER && CenterGamePanel.repainterThread.isLastShotForTimeUp()==false){
	            		if(GameManager.isPause()==false)
	            		{
	            			AudioProvider.pause();
	            			setImageMenu();
	...
	public static void load(int myPlayer,int comPlayer,int playGround)
	{
	  ...
	  public static double getxShoot() {
			return xShoot;
		}
	  
		public static void setxShoot(double xShoot) {
			CenterGamePanel.xShoot += xShoot;
		}

		public static double getyShoot() {
			return yShoot;
		}

		public static void setyShoot(double yShoot) {
	...
	protected  void paintComponent(Graphics g) {
		...
		// disc
		g.drawImage(frisbeeImage,(int)gameManager.getDisc().getX(),(int)gameManager.getDisc().getY(),dimensionOfDisc,dimensionOfDisc,this); 
		// My player
		g.drawImage(myPlayerImage,(int)gameManager.getMyPlayer().getX(),(int)gameManager.getMyPlayer().getY(),this);
		// COM player
		g.drawImage(comPlayerImage,(int)gameManager.getComPlayer().getX(),(int)gameManager.getComPlayer().getY(),this);
		//event
...
//			g.drawImage(pointsImage, 1, gameManager.getDisc().getY(),this);
		...
		//shadow
		g.drawImage(shadow,(int)gameManager.getMyPlayer().getX()+((int)(width*0.021)),(int)gameManager.getMyPlayer().getY()+((int)(width*0.095)),this);
		g.drawImage(shadow,(int)gameManager.getComPlayer().getX()+((int)(width*0.021)),(int)gameManager.getComPlayer().getY()+((int)(width*0.095)),this);
